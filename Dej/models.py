...
class PetitDej(models.Model):
    ...
    def participants(self):
        queryset = [x for x in self.possible_users if not x in self.organisateurs]
        queryset = [x for x in queryset if not x in self.absents.all()]
        return queryset
    ...
    def tirage_aleatoire(self):
        ...
        min_points = min(list(user_points.values()))  # Si tous le monde a des points positifs, on prend la personne ayant le moins de points.
        real_min_points = max(min_points, 0)
        records = [{"email": email, "points": points} for email, points in user_points.items() if points <=real_min_points]
